...
public class TuanDAO extends SuperDAO {
	 ...
	 * firm:"南方高新火锅城",sta:1234567891011,end:1234567891012,count:10,remain:5}
	 */
	public JSONObject getQueryTuan(int tuanID){
		
		JSONObject tuanJson = new  JSONObject();
		
		PosLogger.log.debug("Get one Tuan,tuanID=" + tuanID);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and t.tuan_id=" + tuanID);
	...
	/**
	 * @author penglang
	 * @param tuanID(团购名称关键字),start(开始位置),count(查询条数)
	 * @return JsonObject
	 * 获取搜索匹配输入的团购名称关键字的团购信息
	 * json说明:"list"-包含所有日志的数组名,"count"-实际得到的日志条数,"cat"-团购类别名称,"tid"-团购ID
	 * "name"-团购名字,"desc"-团购描述,"firm"-团购商家名称,"sta"-团购开始时间,"end"-团购结束时间,
	 * "count"-该团购总兑换码数量,"remain"-该团购剩余兑换码数量
	 * json例:{count:2,list:[{cat:"餐饮",tid:1,name:"南方高新火锅城团购券",desc:"南方高新火锅城70一人随便吃",
	 ...
	 * firm:"南方高新台球俱乐部",sta:1234567891011,end:1234567891012,count:10,remain:5}]}
	 */
	public JSONObject getQueryTuanByTuanName(String tuanName,int start,int count){
		
		JSONObject tuanArray = new JSONObject();
		
		PosLogger.log.debug("Get query Tuan,tuanName=" + tuanName);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and t.tuan_Name like '%" + tuanName + "%' ORDER BY t.tuan_starttime desc " +
							"limit " + start + "," + count);
	...
	/**
	 * @author penglang
	 * @param tuanDesc(团购描述关键字),start(开始位置),count(查询条数)
	 * @return JsonObject
	 * 获取搜索匹配输入的团购描述关键字的团购信息
	 * json说明:"list"-包含所有日志的数组名,"count"-实际得到的日志条数,"cat"-团购类别名称,"tid"-团购ID
	 * "name"-团购名字,"desc"-团购描述,"firm"-团购商家名称,"sta"-团购开始时间,"end"-团购结束时间,
	 * "count"-该团购总兑换码数量,"remain"-该团购剩余兑换码数量
	 * json例:{count:2,list:[{cat:"餐饮",tid:1,name:"南方高新火锅城团购券",desc:"南方高新火锅城70一人随便吃",
	 ...
	 * firm:"南方高新台球俱乐部",sta:1234567891011,end:1234567891012,count:10,remain:5}]}
	 */
	public JSONObject getQueryTuanByTuanDesc(String tuanDesc,int start,int count){
		
		JSONObject tuanArray = new JSONObject();
		
		PosLogger.log.debug("Get query Tuan,tuanDesc=" + tuanDesc);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and t.tuan_desc like '%" + tuanDesc + "%' ORDER BY t.tuan_starttime desc " +
							"limit " + start + "," + count);
	 ...
	 * firm:"南方高新台球俱乐部",sta:1234567891011,end:1234567891012,count:10,remain:5}]}
	 */
	public JSONObject getQueryTuanByFirmName(String firmName,int start,int count){
		
		JSONObject tuanArray = new JSONObject();
		
		PosLogger.log.debug("Get query Tuan,firmName=" + firmName);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and f.firm_name like '%" + firmName + "%' ORDER BY t.tuan_starttime desc " +
							"limit " + start + "," + count);
	...
	/**
	 * @author penglang
	 * @param categoryName(类别名称关键字),start(开始位置),count(查询条数)
	 * @return JsonObject
	 * 获取搜索匹配输入的团购描述关键字的团购信息
	 * json说明:"list"-包含所有日志的数组名,"count"-实际得到的日志条数,"cat"-团购类别名称,"tid"-团购ID
	 * "name"-团购名字,"desc"-团购描述,"firm"-团购商家名称,"sta"-团购开始时间,"end"-团购结束时间,
	 ...
	 * firm:"南方高新台球俱乐部",sta:1234567891011,end:1234567891012,count:10,remain:5}]}
	 */
	public JSONObject getQueryTuanByCategoryName(String categoryName,int start,int count){
		
		JSONObject tuanArray = new JSONObject();
		
		PosLogger.log.debug("Get query Tuan,categoryName=" + categoryName);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and c.category_name like '%" + categoryName + "%' ORDER BY t.tuan_starttime desc " +
							"limit " + start + "," + count);
	...
	/**
	 * @author penglang
	 * @param categoryID(类别ID),start(开始位置),count(查询条数)
	 * @return JsonObject
	 * 获取类别下的团购信息
	 * json说明:"list"-包含所有日志的数组名,"count"-实际得到的日志条数,"cat"-团购类别名称,"tid"-团购ID
	 * "name"-团购名字,"desc"-团购描述,"firm"-团购商家名称,"sta"-团购开始时间,"end"-团购结束时间,
	 ...
	 * firm:"南方高新台球俱乐部",sta:1234567891011,end:1234567891012,count:10,remain:5}]}
	 */
	public JSONObject getTuanByCategoryID(int categoryID,int start,int count){
		
		JSONObject tuanArray = new JSONObject();
		
		PosLogger.log.debug("Get query Tuan,categoryID=" + categoryID);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and t.category_id=" + categoryID + " ORDER BY t.tuan_starttime desc " +
							"limit " + start + "," + count);
	...
	/**
	 * @author penglang
	 * @param firmID(商家ID),start(开始位置),count(查询条数)
	 * @return JsonObject
	 * 获取商家下的团购信息
	 * json说明:"list"-包含所有日志的数组名,"count"-实际得到的日志条数,"cat"-团购类别名称,"tid"-团购ID
	 * "name"-团购名字,"desc"-团购描述,"firm"-团购商家名称,"sta"-团购开始时间,"end"-团购结束时间,
	 ...
	 * firm:"南方高新台球俱乐部",sta:1234567891011,end:1234567891012,count:10,remain:5}]}
	 */
	public JSONObject getTuanByfirmID(int firmID,int start,int count){
		
		JSONObject tuanArray = new JSONObject();
		
		PosLogger.log.debug("Get query Tuan,firmID=" + firmID);
		
		conn = SQLConn.getConnection();
		
		try {
			ps = conn.prepareStatement("SELECT * FROM temp_firm f,temp_tuan t,temp_category c " +
					" WHERE t.category_id=c.category_id AND f.firm_id=t.firm_id and t.firm_id=" + firmID + " ORDER BY t.tuan_starttime desc " +
							"limit " + start + "," + count);
